## grequests解读
之前遇到一个需要批量访问url的需求，当时对协程、多线程这些还不太了解，便找了这个库来实现。
今天看了一下grequests的源码，发现只有一个文件，短短100多行。不过还是有很多值得学习的地方。
```
...
print(resp)
```
- 需要深挖的点： gevent.pool、session、pool.spawn、pool.imap_unordered、patch_all
todo

- 要学习的写法, 内部包好，外部直接文件名.方法，简单易用
```
get = partial(AsyncRequest, 'GET')
options = partial(AsyncRequest, 'OPTIONS')
head = partial(AsyncRequest, 'HEAD')
